...
static BaseTag* GetPresetTag() {
    if (!g_preset_tag) {
        Filename filename = GeGetPluginPath() + "presets" + XPRESSOEFFECTOR_PRESETNAME;
...
public:
    ...
    virtual void ModifyPoints(BaseObject* op, BaseObject* gen, BaseDocument* doc,
        ...
        m_execInfo.easyValues = (Vector*) GeAlloc(sizeof(Vector) * md->GetCount());

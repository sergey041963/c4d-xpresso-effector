...
Real VectorLengthInterpolate(const Vector& a, const Vector& b, Real weight) {
    Real base = a.GetLengthSquared();
    return Sqrt(base + (b.GetLengthSquared() - base) * weight);
}
...
Bool RegisterCSVEffector() {
    Bool ok = RegisterEffectorPlugin(
            ...
            "Ocsveffector",
            AutoBitmap("Ocsveffector.tif"),
            CSVEFFECTOR_VERSION);
    if (!ok) {
        GePrint(GeLoadString(IDC_CSVEFFECTOR_NOTREGISTERED));
    }
    return ok;
}
